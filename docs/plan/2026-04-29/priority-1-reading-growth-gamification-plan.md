...
8. 성장 레벨은 계산 점수 구간으로 숫자만 산출하되, UI에는 레벨명 대신 대표 배지와 진행률을 노출한다.
9. 배지는 조건 충족 여부를 계산해 응답하고, 대표 배지는 우선순위 규칙으로 1개를 선택한다.
...
레벨 숫자는 내부 진행도 계산과 다음 배지까지의 진행률 산출에만 사용한다. 사용자 화면에는 레벨명을 별도 문자열로 노출하지 않는다.
...
2. 대표 배지, 진행률, 이번 달 읽은 책, 저장 후 읽음 전환, 카테고리 다양성을 표시한다.
...
- `docs/design-concept.md`의 조용함, 신뢰감, 지적인 느낌을 유지한다.
- 성장 카드는 경쟁형 보상 UI가 아니라 마이페이지의 개인 요약 정보로 배치한다.
- 주요 색상은 Deep Blue `#1E2A38`, Soft Beige `#F5F3EF`, Muted Green `#4CAF50`, Warm Orange `#F59E0B`를 사용한다.
- 점수 숫자와 레벨명을 크게 노출하지 않고, 대표 배지, 진행률, 설명 문구를 중심으로 보여준다.

### Desktop: 마이페이지 성장 카드 배치
...
│ 이메일 / 계정 상태           │ │ 경제 입문자                               │ │
│                              │ │ 경제 분야와 투자 키워드를 중심으로       │ │
│ [관심 3] [목적 2]            │ │ 취향이 선명해지고 있습니다.              │ │
│ [읽은 책 6] [추천 4]         │ │                                           │ │
│                              │ │ 다음 배지까지                             │ │
│                              │ │ [████████████░░░░░░] 45%                 │ │
│                              │ │                                           │ │
│                              │ │ ┌─────────┬─────────┬─────────┬────────┐ │ │
│                              │ │ │이번 달  │저장→읽음│분야 다양│추천 전환│ │ │
│                              │ │ │3권      │1권      │2개      │1권     │ │ │
│                              │ │ └─────────┴─────────┴─────────┴────────┘ │ │
│                              │ │                                           │ │
│                              │ │ 배지                                      │ │
...
### Component: ReadingGrowthCard

```text
ReadingGrowthCard
├── Section Label: 독서 성장
├── Primary Badge: 경제 입문자
├── Summary
├── Progress Bar
├── Metric Grid
│   ├── monthlyReadCount
