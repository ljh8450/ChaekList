...
- 모각독의 최소 영속 모델과 API 응답 계약을 먼저 고정한다.

작업:

- `domain/readingroom` 패키지를 추가한다.
- `ReadingRoomController`, `ReadingRoomService`, `ReadingRoomDtos`를 기존 `SocialController`/`SocialService` 스타일에 맞춰 구성한다.
- `reading_rooms`, `reading_room_participants`, `reading_room_checkins` schema를 추가한다.
- 방 상태 값은 저장 상태와 현재 시간 기반 표시 상태를 구분할지 결정한다.
- 책 요약 응답은 기존 `BookSummaryResponse` 또는 Social의 `BookSummary` 재사용 가능성을 먼저 확인한다.
...
- 방 생성 서비스 단위 테스트
- 방장 자동 참여 테스트
- 잘못된 시간, 정원, 누락된 책 id 요청 거부 테스트

### 2단계: 조회 API

목표:

- frontend가 화면을 만들 수 있는 읽기 전용 API를 먼저 안정화한다.
...
- 응답에 `participantCount`, `maxParticipants`, `myParticipationStatus`, `canJoin`, `canCancel`, `canCheckIn`을 포함할지 검토한다.

검증:

- 비로그인 공개 조회 테스트
- 로그인 사용자의 `myParticipationStatus` 포함 테스트
- 관리자 숨김/취소 방 공개 목록 제외 테스트
- 책별 조회에서 해당 책 방만 반환되는지 테스트

### 3단계: 참여와 인증

목표:

- MVP의 핵심 사용자 행동인 참여, 취소, 종료 후 인증을 구현한다.
...
- `POST /api/reading-rooms/{roomId}/checkins`
- 참여 중복 요청은 멱등 처리한다.
- 같은 시간대 중복 참여 제한을 서비스 레벨에서 검사한다.
- 인증 성공 시 참여 상태를 `COMPLETED`로 전환한다.
...
- 종료 전 인증 거부 테스트
- 중복 인증 거부 테스트
- 인증 후 `COMPLETED` 전환 테스트

### 4단계: 성장/배지 연동

목표:

- 모각독 완료 이력을 독서 성장의 보조 신호로 반영한다.

작업:

- `MyPageService`의 성장 계산에 모각독 월간 보조 점수를 추가한다.
- 월간 최대 6점 상한을 적용한다.
- 인증 완료 기준으로만 점수를 계산한다.
- 모각독 배지 후보 중 MVP에 필요한 최소 배지를 추가한다.
...
- 월간 점수 상한 테스트
- 취소/숨김 방 제외 테스트
- 대표 배지 선택 우선순위가 기존 배지를 과도하게 밀어내지 않는지 테스트

### 5단계: Social, moderation, 알림 연동

목표:

- 기존 Social 정책을 재사용하되 MVP 범위를 넘지 않게 연결한다.

작업:

- 모각독 완료 공유 타입 또는 기존 게시글 타입 확장 방식을 결정한다.
- 공유 게시글 응답이 feed/search/profile의 공개/숨김 정책을 따르게 한다.
- 모각독 방 신고 target type 확장 또는 별도 신고 저장소를 결정한다.
- 저장형 알림은 스케줄러 없이 새 참여자 알림부터 검토한다.

검증:

- 모각독 공유 게시글 feed 노출 테스트
- 비공개/숨김/탈퇴 사용자 정책 회귀 테스트
- 신고된 방 또는 숨김 방의 공개 목록 제외 테스트
- 알림 설정이 꺼진 사용자에게 알림을 생성하지 않는지 테스트
...
- backend API 계약에 맞춰 화면들이 공통으로 사용할 최소 호출 흐름을 만든다.

작업:

- `App.jsx`에 `/reading-rooms`, `/reading-rooms/:roomId`, `/me/reading-rooms` 라우트 후보를 추가한다.
- 보호가 필요한 화면은 기존 `RequireAuth`를 재사용한다.
- 모각독 API 호출은 우선 각 페이지 내부 함수로 시작하고, 중복이 커질 때만 helper 분리를 검토한다.
- 인증 header 처리는 기존 페이지의 `useAuth()` 패턴을 따른다.

검증:

- 비로그인 사용자가 목록/상세를 볼 수 있는지 확인
- 로그인 필요 액션에서 로그인 페이지 이동 또는 오류 안내가 동작하는지 확인
- 존재하지 않는 방 id의 오류 상태가 깨지지 않는지 확인

### 2단계: 모각독 목록 화면

목표:

- 사용자가 현재 열려 있는 온라인 모각독을 탐색할 수 있게 한다.

작업:

- `ReadingRoomsPage.jsx`를 추가한다.
- 모집 중/진행 예정/종료 상태 필터를 제공한다.
- 방 카드에는 책 제목, 방 제목, 시작/종료 시간, 참여자 수, 상태를 표시한다.
- 빈 상태와 로딩 상태를 기존 화면 톤에 맞춰 처리한다.

검증:

- `/reading-rooms`에서 목록 렌더링 확인
- 필터 변경 시 query와 목록 갱신 확인
- 긴 방 제목과 긴 책 제목이 카드 레이아웃을 깨지 않는지 확인

### 3단계: 책 상세 연동과 방 생성

목표:

- 책 상세에서 자연스럽게 해당 책 기준 모각독으로 이어지게 한다.

작업:

- `BookDetailPage.jsx`에 해당 책의 모각독 섹션을 추가한다.
- `이 책으로 함께 읽기 열기` 액션을 추가한다.
- 방 생성 UI는 별도 페이지 또는 모달 중 기존 화면 복잡도가 낮은 쪽으로 결정한다.
- 생성 필드는 제목, 설명, 시작/종료 시간, 최대 인원만 둔다.

검증:

- `/books/:bookId`에서 해당 책 방만 표시되는지 확인
- 로그인하지 않은 사용자가 생성 액션을 누르면 인증 흐름으로 이동하는지 확인
- 필수값 누락, 종료 시간이 시작 시간보다 빠른 경우 안내 확인

### 4단계: 방 상세, 참여, 취소

목표:

- 방 상세에서 사용자가 참여 가능 여부를 명확히 판단하고 액션을 수행하게 한다.

작업:

- `ReadingRoomDetailPage.jsx`를 추가한다.
- 방 정보, 책 요약, 시간, 정원, 상태, 내 참여 상태를 표시한다.
- `참여하기`, `참여 취소` 버튼을 상태에 따라 노출한다.
- 참여/취소 후 상세 데이터를 다시 불러와 화면 상태를 맞춘다.

검증:

- 참여 가능 방의 참여 버튼 동작 확인
- 이미 참여한 방의 취소 버튼 동작 확인
- 정원 초과/시작 후 취소 실패 메시지 확인
- 새로고침 후에도 내 참여 상태가 유지되는지 확인

### 5단계: 종료 후 인증과 내 모각독

목표:

- 사용자가 완료해야 할 인증을 놓치지 않고 처리하게 한다.

작업:

- 방 종료 후 `note` 또는 `progress` 인증 입력을 표시한다.
- 종료 전에는 인증 입력을 비활성화한다.
- `MyReadingRoomsPage.jsx`에서 참여 중, 인증 필요, 완료 상태를 구분한다.
- 마이페이지에 바로 붙이는 것은 후순위로 두고 독립 화면을 먼저 안정화한다.

검증:

- 종료 전 인증 비활성 확인
- 종료 후 인증 성공과 완료 상태 전환 확인
- 중복 인증 실패 안내 확인
- 내 모각독 목록에서 인증 필요 방이 우선 보이는지 확인

### 6단계: 공유 카드와 성장 표시

목표:

- 모각독 완료 경험을 Social feed와 독서 성장 화면에 연결한다.

작업:

- `SocialPostCard.jsx`에 모각독 공유 타입 렌더링을 추가한다.
- 모각독 완료 후 공유 액션을 제공한다.
- `MyPage.jsx`에는 backend 성장 응답에 포함된 배지/점수를 기존 카드가 자연스럽게 표시하는지 먼저 확인한다.
- 별도 모각독 배지 섹션은 필요성이 확인될 때만 추가한다.

검증:

- 공유 생성 후 `/social`에서 카드 표시 확인
- 공유 게시글 공개/비공개 전환이 기존 버튼과 충돌하지 않는지 확인
- 마이페이지 독서 성장 카드가 기존 레이아웃을 깨지 않는지 확인
...
1. Backend 1단계 데이터와 도메인 기반
2. Backend 2단계 조회 API
3. Frontend 1단계 라우팅과 API 호출 기반
4. Frontend 2단계 모각독 목록 화면
5. Frontend 3단계 책 상세 연동과 방 생성
6. Backend 3단계 참여와 인증
7. Frontend 4단계 방 상세, 참여, 취소
8. Frontend 5단계 종료 후 인증과 내 모각독
9. Backend 4단계 성장/배지 연동
10. Frontend 6단계 공유 카드와 성장 표시
11. Backend 5단계 Social, moderation, 알림 연동
