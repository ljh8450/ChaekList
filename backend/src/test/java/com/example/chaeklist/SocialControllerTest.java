...
class SocialControllerTest {
	...
	private long insertCompletedReadingRoom(long userId, long bookId) {
		jdbcTemplate.update("""
				...
				VALUES (?, ?, '완료 공유 모각독', DATEADD('HOUR', -2, CURRENT_TIMESTAMP), DATEADD('HOUR', -1, CURRENT_TIMESTAMP), 5, 'RECRUITING', 'PUBLIC', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
				""", userId, bookId);
		long roomId = jdbcTemplate.queryForObject("SELECT MAX(id) FROM reading_rooms", Long.class);
		jdbcTemplate.update("""
				INSERT INTO reading_room_participants (room_id, user_id, status, joined_at, completed_at)
				VALUES (?, ?, 'COMPLETED', DATEADD('HOUR', -2, CURRENT_TIMESTAMP), DATEADD('HOUR', -1, CURRENT_TIMESTAMP))
				""", roomId, userId);
		return roomId;
	}
}
